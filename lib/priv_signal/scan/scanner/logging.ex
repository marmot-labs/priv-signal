defmodule PrivSignal.Scan.Scanner.Logging do
  ...
  alias PrivSignal.Scan.Scanner.Evidence, as: EvidenceCollector
  ...
  defp collect_evidence(node, %Inventory{} = inventory) do
    direct = EvidenceCollector.collect(node, inventory)
        ...
        {:inspect, _, [arg]} ->
          case arg do
            {name, _, context} when is_atom(name) and is_atom(context) ->
              if possible_bulk_variable?(name) do
                [
                  %Evidence{
                    type: :bulk_inspect,
                    expression: "bulk_inspect:" <> Atom.to_string(name),
                    fields: pseudo_fields(inventory)
                  }
                ]
              else
                []
