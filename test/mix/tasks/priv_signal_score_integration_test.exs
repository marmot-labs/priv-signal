defmodule Mix.Tasks.PrivSignal.ScoreIntegrationTest do
  ...
  test "runs pipeline with mocked LLM" do
    tmp_dir = Path.join(System.tmp_dir!(), "priv_signal_pipeline_#{System.unique_integer([:positive])}")
    ...
    try do
      ...
      assert {:ok, _path} = PrivSignal.Output.Writer.write(markdown, json, json_path: "priv-signal.json", quiet: true)
